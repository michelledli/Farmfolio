...
const handleSearch = () => { 
  api.getSearchByNameOrTag(search)
    .then(response => { 
...
return (
  <div className="App-header">
    ...
    <button onClick={()=>Navigate("/animal-add")}className="addButton">Add Animal</button>
      <label>Search:</label>
    ...
    <div className="instructionalText">
      <p>
        Filter by:
        <ul>
          <li>Click on table headers to sort by name, age, weight, tag, or breed.</li>
        </ul>
      </p>
      <p>
        Get filtered results:
        <ul>
          <li>Click on the table headers to sort the data in ascending or descending order.</li>
        </ul>
      </p>
      <p>
        Search by:
        <ul>
          <li>Enter a search query in the input field above to find livestock by name or tag.</li>
        </ul>
      </p>
    </div>
          ...
          <tbody>
            {sortedGoats.map((goat) => (
              ...
              <td style={{ color: "black", fontSize: "17px"}}>
                <Link to={`/livestock/${goat.id}`}className="link-no-underline">{goat.name}</Link>
              </td>
              <td style={{ color: "black", fontSize: "17px" }}>{getAge(goat.dob)}</td>
